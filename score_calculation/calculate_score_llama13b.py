...
if __name__ == '__main__':
    ...
    model_name = 'Llama-2-13b-chat-hf-api-llamaprompt-original-float32'
    ...
    for prompt_type in os.listdir(file_location):
        ...
        if sum([len(scores) for scores in score_list.values()]):
            for trait in score_list:
                ...
                print(model_name, prompt_type.replace('.json', ''), trait, '$' + str(mean_score) + '_{' + str(std) + '}$', 'MOST COMMON:', most_common_score, 'LEN:', len(score_list[trait]), 'A:', option_list[trait])